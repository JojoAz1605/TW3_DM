...
if (!isset($_POST["author"]) && !isset($_POST["title"]) && !isset($_POST["descriptionP"]) && !isset($_POST["dateP"]) && !isset($_FILES["photo"])) {
    ...
    $errors = check_errors($photo->get_author(), $photo->get_descriptionP(), $photo->get_dateP(), $file);

    if(count_errors($errors) == 0) {  // s'il n'y a pas d'erreurs
        add_to_database($photo, $file);
        $body .= "<h2>Photo ajoutée!</h2>";
    } else {include_once("public/php/pages/formulaire.php"); }
}

function check_errors($author, $descriptionP, $dateP, $file): array {
...
function add_to_database(Photo $photo, $file): void {
